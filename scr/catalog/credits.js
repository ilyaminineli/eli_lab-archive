document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        const draw = () => {
            ...
            const visible = entities.filter(entity => {
                const hay = [
                    ...
                    entity.kind,
                    ...(entity.source_forms || []),
                    ...(entity.roles_observed || []),
                    entity.notes,
                    entity.persona_notes
                ].join(' ').toLowerCase();

                return (kindFilter === 'all' || entity.kind === kindFilter) &&
            ...
            list.innerHTML = visible.map(entity => {
                ...
                return '<article class="credit-card">' +
                    '<header><div><span class="credit-id">' + escapeHTML(entity.canonical_id || 'source') +
                    '</span><h2>' + escapeHTML(entity.source_forms?.[0] || entity.canonical_id) +
                    '</h2></div><span class="credit-kind">' + escapeHTML(entity.kind || 'source') +
                    '</span></header>' +
                    '<div class="credit-row"><span>SOURCE FORMS</span><p>' +
                    escapeHTML((entity.source_forms || []).join(' · ') || '—') + '</p></div>' +
                    '<div class="credit-row"><span>ROLES OBSERVED</span><p>' +
                    escapeHTML((entity.roles_observed || []).join(' · ') || '—') + '</p></div>' +
                    (entity.persona_notes ? '<div class="credit-row"><span>PERSONA NOTE</span><p>' +
                        escapeHTML(entity.persona_notes) + '</p></div>' : '') +
                    (entity.notes ? '<div class="credit-row"><span>NOTE</span><p>' +
                        escapeHTML(entity.notes) + '</p></div>' : '') +
                    '<a class="credit-open" href="' + target + '">OPEN RELATED INDEX ↗</a>' +
                    '</article>';
            }).join('') || '<p class="small-note">NO SOURCE OBSERVATIONS MATCH.</p>';

            if (count) count.textContent = String(visible.length).padStart(3, '0') +
                ' / ' + String(entities.length).padStart(3, '0') + ' SOURCE ENTITIES';
        };
        ...
        const writeUrl = () => {
            ...
            if (kind?.value && kind.value !== 'all') next.searchParams.set('kind', kind.value);
